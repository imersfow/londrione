  ...
  return (
    <div className="public-home min-h-screen text-slate-900" style={themeCssVars(theme)}>
      <header className="sticky top-0 z-40 border-b border-white/70 bg-white/85 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            ...
            {onlineOrderHref && <Link href={onlineOrderHref} className="public-btn-primary"><Bike size={16}/> Order Online</Link>}
            ...
            {wa && <a href={wa} target="_blank" rel="noreferrer" className="public-btn-secondary"><MessageCircle size={16}/> WhatsApp</a>}
          </div>
        </div>
      </header>
      ...
      <main>
          ...
          <div className="mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1.2fr_.8fr] lg:px-8 lg:py-24">
            <div className="flex flex-col justify-center">
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                {onlineOrderHref && <Link href={onlineOrderHref} className="public-btn-primary"><Bike size={17}/>Order Online</Link>}
                {config.show_prices && <a href="#prices" className="public-btn-secondary">{config.hero_cta_label}<ArrowRight size={17}/></a>}
        ...
        {config.show_contact && <section className="public-section"><div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8"><div className="public-contact-card"><div><div className="text-3xl font-black text-white">{config.contact_title}</div><p className="mt-2 max-w-2xl text-white/75">{hasMultipleBranches ? config.contact_text : config.contact_text.replace(/Pilih cabang dan /i, "")}</p>{branchAddress(branch) && <div className="mt-4 flex items-center gap-2 text-sm text-white/70"><MapPin size={15}/>{branchAddress(branch)}</div>}</div><div className="flex flex-wrap gap-2">{onlineOrderHref && <Link href={onlineOrderHref} className="rounded-2xl bg-white px-5 py-3 font-black text-slate-900 shadow-lg"><span className="inline-flex items-center gap-2"><Bike size={18}/>Order Online</span></Link>}{wa && <a href={wa} target="_blank" rel="noreferrer" className="rounded-2xl bg-white/90 px-5 py-3 font-black text-slate-900 shadow-lg"><span className="inline-flex items-center gap-2"><MessageCircle size={18}/>WhatsApp</span></a>}</div></div></div></section>}
      </main>
