...
export default function ProductionPage(){
  ...
  useEffect(()=>{load()},[]);
  async function move(id:string,status:string){const{error}=await supabase.rpc("update_order_status",{p_order_id:id,p_status:status});if(!error)kickNotificationWorker(12);load()}
  ...
  <div className="grid gap-4 xl:grid-cols-3 2xl:grid-cols-6">{lanes.map((s,idx)=><div key={s} className={`theme-card theme-card-${idx+1} min-h-52 p-3`}><div className="mb-3 flex items-center justify-between"><div className="text-sm font-black">{statusLabel[s]}</div><span className="badge-neutral">{grouped[s]?.length||0}</span></div><div className="space-y-2">{(grouped[s]??[]).map((o:any)=><div key={o.id} className="rounded-2xl border border-white/80 bg-white/60 p-3 shadow-sm backdrop-blur-sm"><Link href={`/orders/${o.id}`} className="font-bold text-violet-700">{o.order_number}</Link><div className="mt-1 text-sm font-semibold">{o.customer_name}</div><div className="text-xs text-slate-500">{o.branches?.name||"-"}</div>{idx<lanes.length-1&&<button onClick={()=>move(o.id,lanes[idx+1])} className="mt-3 flex w-full items-center justify-center gap-1 rounded-lg bg-slate-900 px-2 py-2 text-xs font-semibold text-white">Lanjut <ArrowRight size={13}/></button>}</div>)}{!grouped[s]?.length&&<div className="rounded-2xl border border-dashed border-white/90 bg-white/30 p-6 text-center text-xs text-slate-400">Kosong</div>}</div></div>)}</div></div>
}
